...
**Study Design**: Retrospective cohort study using MIMIC-III
database

**Primary Analysis**: Association between first lactate measurement
...
Multiple Imputation by Chained Equations is a robust, informative method of dealing with missing data in
datasets. The procedure ‘fills in’ (imputes) missing data in a dataset through an iterative series of predictive
models. In each iteration, each specified variable in the dataset is imputed using the other variables in the
dataset. These iterations should be run until it appears that convergence has been met.

**Data Leakage**:

MICE is particularly useful if missing values are associated with the target variable in a way that introduces
leakage. For instance, let’s say you wanted to model customer retention at the time of sign up. A certain
variable is collected at sign up or 1 month after sign up. The absence of that variable is a data leak, since
it tells you that the customer did not retain for 1 month.

**Funnel Analysis**:

Information is often collected at different stages of a ‘funnel’. MICE can be used to make educated guesses
about the characteristics of entities at different points in a funnel.

**Confidence Intervals**:
MICE can be used to impute missing values, however it is important to keep in mind that these imputed
values are a prediction. Creating multiple datasets with different imputed values allows you to do two types
of inference:

* Imputed Value Distribution: A profile can be built for each imputed value, allowing you to make
statements about the likely distribution of that value.

* Model Prediction Distribution: With multiple datasets, you can build multiple models and create a
distribution of predictions for each sample. Those samples with imputed values which were not able
to be imputed with much confidence would have a larger variance in their predictions.

> defaultMethod = c(“pmm”, “logreg”, “polyreg”, “polr”) A vector of length 4 containing the
default imputation methods for 1) numeric data, 2) factor data with 2 levels, 3) factor data with
> 2 unordered levels, and 4) factor data with > 2 ordered levels. By default, the method uses
16 pmm, predictive mean matching (numeric data) logreg, logistic regression imputation (binary
data, factor with 2 levels) polyreg, polytomous regression imputation for unordered categorical
data (factor > 2 levels) polr, proportional odds model for (ordered, > 2 levels).

```{r}
# missing_columns 
# imputed_data <- mice(eda_df,
#                       m = 5,
#                       maxit = 50,
#                       method = 'pmm',
#                       seed = 42,
...
EXPLAIN WHY WE CANNOT TO AFFORD THIS SECTION DUE DATA LEAKAGE

##############

```{r}
complete_data <- eda_df %>% 
  na.omit()
cat("Complete cases:", nrow(complete_data), "of", nrow(clean_values_df),
"(", round(100*nrow(complete_data)/nrow(clean_values_df), 2), "%)\n")
```
...
## Primary analysis (hypothesis testing, estimations, descriptive analysis, probability, confidence interval, CLT)
...
```{r}
# Test association between lactate categories and mortality (Chi-square)
chisq_result <- chisq.test(
  table(stats_df$lactate_flag, stats_df$icu_dead)
)
```


```{r}
cat(sprintf("\nChi-square test: lactate category vs mortality\n"))
cat(sprintf("χ² = %.2f, df = %d, p < 0.001\n", 
            chisq_result$statistic, chisq_result$parameter))
```

```{r}
describe(initial_df$lactate_value_num)
```
...
modeest::mfv(stats_df$age_icu_entry)

ggplot(stats_df, aes(x = age_icu_entry)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30,
                  fill = "skyblue", color = "black") +
geom_density(color = "red", size = 1.5) +
geom_vline(xintercept = mean(stats_df$age_icu_entry),
          color = "blue", linetype = "dashed", size = 1) +
geom_vline(xintercept = median(stats_df$age_icu_entry),
          color = "darkgreen", linetype = "dashed", size = 1) +
geom_vline(xintercept = modeest::mfv(stats_df$age_icu_entry),
          color = "purple", linetype = "dashed", size = 1) +
labs(title = "Distribución de Edad",
subtitle = paste("Media =", round(mean(stats_df$age_icu_entry), 1), "años",
                 "\nMediana =", round(median(stats_df$age_icu_entry), 1), "años",
                  "\nModa =", round(modeest::mfv(stats_df$age_icu_entry), 1), "años"),
x = "Edad (años)",
y = "Densidad"
) +
theme_minimal() +
...
```{r}
stats_df %>%
  filter(age_icu_entry > 100)
```

```{r}
# Extraeremos los estadísticos por edad
stats_df %>%
  group_by(lactate_flag) %>%
  summarise(
    count = n(),
    mean_age = mean(age_icu_entry, na.rm = TRUE),
    median_age = median(age_icu_entry, na.rm = TRUE),
    sd_age = sd(age_icu_entry, na.rm = TRUE),
    min_age = min(age_icu_entry, na.rm = TRUE),
    max_age = max(age_icu_entry, na.rm = TRUE)
  )
```

```{r}
ggplot(stats_df %>% filter(age_icu_entry < 100), aes(x = factor(lactate_flag, labels = c("normal", "abnormal")),
y = age_icu_entry, fill = factor(lactate_flag))) +
...
corrplot::corrplot(stats_df$lactate_value_num)
```
...
# Convert lactate category to numeric scores

# lactate_scores 
#   "Normal (<2 mmol/L)" ~ 0,
#   "Elevated (2-4 mmol/L)" ~ 1,
#   "Severe (≥4 mmol/L)" ~ 2
...
```{r}
trend_test <- cor.test(stats_df$lactate_scores, stats_df$icu_dead, 
                       method = "spearman")
cat(sprintf("\nSpearman correlation: ρ = %.3f, p < 0.001\n", 
            trend_test$estimate))
```


```{r}
# Univariable logistic regression for continuous lactate
univar_model <- glm(
  icu_dead ~ lactate_scores,
  data = stats_df,
  family = binomial()
)

cat("\nUnivariate Logistic Regression: Mortality ~ Lactate (continuous)\n")
print(tidy(univar_model, exponentiate = TRUE, conf.int = TRUE))
```
...
# Machine Learning modeling  (clustering logistic regression, lineal model, etc., ROC curves)
